...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!examName || !date || !time || !duration || !totalMarks || !passingMarks || !questionDistribution) {
...
router.get('/exams', async (req, res) => {
  try {
    const exams = await Examination.find({})
      .populate('questionDistribution.subject', 'name'); // only real field
    res.json(exams);
  } catch (err) {
...
router.get('/report', async (req, res) => {
  try {
    ...
    const report = exams.map(exam => ({
      ...
      examineeName: exam.examineeId?.firstname + ' ' + exam.examineeId?.lastname || 'N/A',
...
router.post('/result/:id', async (req, res) => {
  ...
  try {
    ...
    if (!exam) return res.status(404).json({ message: "Exam attempt not found" });
...
// Get all pending examination attempts
